/**
 * InvenTree API
 * API for InvenTree - the intuitive open source inventory management system
 *
 * The version of the OpenAPI document: 442
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

/*
 * Owner.h
 *
 * Serializer for an \&quot;Owner\&quot; (either a \&quot;user\&quot; or a \&quot;group\&quot;).
 */

#ifndef Owner_H
#define Owner_H

#include <QJsonObject>
#include <QList>
#include <QMap>

#include <QString>

#include "Enum.h"
#include "HttpFileElement.h"
#include "Object.h"

namespace InvenTree {

class Owner : public Object {
public:
    Owner();
    Owner(const QString &json);
    ~Owner() override;

    QString asJson() const override;
    QJsonObject asJsonObject() const override;
    QMap<QString, QString> asFormVariables() const override;
    QMap<QString, QString> asFormVariables(const QString &namePrefix) const override;
    QList<HttpFileElement> asFileElements() const override;
    QList<HttpFileElement> asFileElements(const QString &namePrefix) const override;
    void fromJsonObject(QJsonObject json) override;
    void fromJson(const QString &jsonString) override;

    qint32 getPk() const;
    void setPk(const qint32 &pk);
    bool is_pk_Set() const;
    bool is_pk_Valid() const;

    qint64 getOwnerId() const;
    void setOwnerId(const qint64 &owner_id);
    bool is_owner_id_Set() const;
    bool is_owner_id_Valid() const;

    QString getOwnerModel() const;
    void setOwnerModel(const QString &owner_model);
    bool is_owner_model_Set() const;
    bool is_owner_model_Valid() const;

    QString getName() const;
    void setName(const QString &name);
    bool is_name_Set() const;
    bool is_name_Valid() const;

    QString getLabel() const;
    void setLabel(const QString &label);
    bool is_label_Set() const;
    bool is_label_Valid() const;

    virtual bool isSet() const override;
    virtual bool isValid() const override;

private:
    void initializeModel();

    qint32 m_pk;
    bool m_pk_isSet;
    bool m_pk_isValid;

    qint64 m_owner_id;
    bool m_owner_id_isSet;
    bool m_owner_id_isValid;

    QString m_owner_model;
    bool m_owner_model_isSet;
    bool m_owner_model_isValid;

    QString m_name;
    bool m_name_isSet;
    bool m_name_isValid;

    QString m_label;
    bool m_label_isSet;
    bool m_label_isValid;
};

} // namespace InvenTree

Q_DECLARE_METATYPE(InvenTree::Owner)

#endif // Owner_H
