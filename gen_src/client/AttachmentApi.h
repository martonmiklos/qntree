/**
 * InvenTree API
 * API for InvenTree - the intuitive open source inventory management system
 *
 * The version of the OpenAPI document: 225
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

#ifndef _AttachmentApi_H
#define _AttachmentApi_H

#include "Helpers.h"
#include "HttpRequest.h"
#include "ServerConfiguration.h"
#include "Oauth.h"

#include "Attachment.h"
#include "PaginatedAttachmentList.h"
#include "PatchedAttachment.h"
#include <QString>

#include <QObject>
#include <QByteArray>
#include <QStringList>
#include <QList>
#include <QNetworkAccessManager>

namespace InvenTree {

class AttachmentApi : public QObject {
    Q_OBJECT

public:
    AttachmentApi(const int timeOut = 0);
    ~AttachmentApi();

    void initializeServerConfigs();
    int setDefaultServerValue(int serverIndex,const QString &operation, const QString &variable,const QString &val);
    void setServerIndex(const QString &operation, int serverIndex);
    void setApiKey(const QString &apiKeyName, const QString &apiKey);
    void setBearerToken(const QString &token);
    void setUsername(const QString &username);
    void setPassword(const QString &password);
    void setTimeOut(const int timeOut);
    void setWorkingDirectory(const QString &path);
    void setNetworkAccessManager(QNetworkAccessManager* manager);
    int addServerConfiguration(const QString &operation, const QUrl &url, const QString &description = "", const QMap<QString, ServerVariable> &variables = QMap<QString, ServerVariable>());
    void setNewServerForAllOperations(const QUrl &url, const QString &description = "", const QMap<QString, ServerVariable> &variables =  QMap<QString, ServerVariable>());
    void setNewServer(const QString &operation, const QUrl &url, const QString &description = "", const QMap<QString, ServerVariable> &variables =  QMap<QString, ServerVariable>());
    void addHeaders(const QString &key, const QString &value);
    void enableRequestCompression();
    void enableResponseCompression();
    void abortRequests();
    QString getParamStylePrefix(const QString &style);
    QString getParamStyleSuffix(const QString &style);
    QString getParamStyleDelimiter(const QString &style, const QString &name, bool isExplode);

    /**
    * @param[in]  attachment Attachment [required]
    */
    virtual void attachmentCreate(const Attachment &attachment);

    /**
    * @param[in]  id qint32 [required]
    */
    virtual void attachmentDestroy(const qint32 &id);

    /**
    * @param[in]  is_file bool [optional]
    * @param[in]  is_link bool [optional]
    * @param[in]  limit qint32 [optional]
    * @param[in]  model_id qint32 [optional]
    * @param[in]  model_type QString [optional]
    * @param[in]  offset qint32 [optional]
    * @param[in]  ordering QString [optional]
    * @param[in]  search QString [optional]
    * @param[in]  upload_user qint32 [optional]
    */
    virtual void attachmentList(const ::InvenTree::OptionalParam<bool> &is_file = ::InvenTree::OptionalParam<bool>(), const ::InvenTree::OptionalParam<bool> &is_link = ::InvenTree::OptionalParam<bool>(), const ::InvenTree::OptionalParam<qint32> &limit = ::InvenTree::OptionalParam<qint32>(), const ::InvenTree::OptionalParam<qint32> &model_id = ::InvenTree::OptionalParam<qint32>(), const ::InvenTree::OptionalParam<QString> &model_type = ::InvenTree::OptionalParam<QString>(), const ::InvenTree::OptionalParam<qint32> &offset = ::InvenTree::OptionalParam<qint32>(), const ::InvenTree::OptionalParam<QString> &ordering = ::InvenTree::OptionalParam<QString>(), const ::InvenTree::OptionalParam<QString> &search = ::InvenTree::OptionalParam<QString>(), const ::InvenTree::OptionalParam<qint32> &upload_user = ::InvenTree::OptionalParam<qint32>());

    /**
    * @param[in]  id qint32 [required]
    */
    virtual void attachmentMetadataPartialUpdate(const qint32 &id);

    /**
    * @param[in]  id qint32 [required]
    */
    virtual void attachmentMetadataRetrieve(const qint32 &id);

    /**
    * @param[in]  id qint32 [required]
    */
    virtual void attachmentMetadataUpdate(const qint32 &id);

    /**
    * @param[in]  id qint32 [required]
    * @param[in]  patched_attachment PatchedAttachment [optional]
    */
    virtual void attachmentPartialUpdate(const qint32 &id, const ::InvenTree::OptionalParam<PatchedAttachment> &patched_attachment = ::InvenTree::OptionalParam<PatchedAttachment>());

    /**
    * @param[in]  id qint32 [required]
    */
    virtual void attachmentRetrieve(const qint32 &id);

    /**
    * @param[in]  id qint32 [required]
    * @param[in]  attachment Attachment [required]
    */
    virtual void attachmentUpdate(const qint32 &id, const Attachment &attachment);


private:
    QMap<QString,int> _serverIndices;
    QMap<QString,QList<ServerConfiguration>> _serverConfigs;
    QMap<QString, QString> _apiKeys;
    QString _bearerToken;
    QString _username;
    QString _password;
    int _timeOut;
    QString _workingDirectory;
    QNetworkAccessManager* _manager;
    QMap<QString, QString> _defaultHeaders;
    bool _isResponseCompressionEnabled;
    bool _isRequestCompressionEnabled;
    HttpRequestInput _latestInput;
    HttpRequestWorker *_latestWorker;
    QStringList _latestScope;
    OauthCode _authFlow;
    OauthImplicit _implicitFlow;
    OauthCredentials _credentialFlow;
    OauthPassword _passwordFlow;
    int _OauthMethod = 0;

    void attachmentCreateCallback(HttpRequestWorker *worker);
    void attachmentDestroyCallback(HttpRequestWorker *worker);
    void attachmentListCallback(HttpRequestWorker *worker);
    void attachmentMetadataPartialUpdateCallback(HttpRequestWorker *worker);
    void attachmentMetadataRetrieveCallback(HttpRequestWorker *worker);
    void attachmentMetadataUpdateCallback(HttpRequestWorker *worker);
    void attachmentPartialUpdateCallback(HttpRequestWorker *worker);
    void attachmentRetrieveCallback(HttpRequestWorker *worker);
    void attachmentUpdateCallback(HttpRequestWorker *worker);

Q_SIGNALS:

    void attachmentCreateSignal(Attachment summary);
    void attachmentDestroySignal();
    void attachmentListSignal(PaginatedAttachmentList summary);
    void attachmentMetadataPartialUpdateSignal();
    void attachmentMetadataRetrieveSignal();
    void attachmentMetadataUpdateSignal();
    void attachmentPartialUpdateSignal(Attachment summary);
    void attachmentRetrieveSignal(Attachment summary);
    void attachmentUpdateSignal(Attachment summary);


    void attachmentCreateSignalFull(HttpRequestWorker *worker, Attachment summary);
    void attachmentDestroySignalFull(HttpRequestWorker *worker);
    void attachmentListSignalFull(HttpRequestWorker *worker, PaginatedAttachmentList summary);
    void attachmentMetadataPartialUpdateSignalFull(HttpRequestWorker *worker);
    void attachmentMetadataRetrieveSignalFull(HttpRequestWorker *worker);
    void attachmentMetadataUpdateSignalFull(HttpRequestWorker *worker);
    void attachmentPartialUpdateSignalFull(HttpRequestWorker *worker, Attachment summary);
    void attachmentRetrieveSignalFull(HttpRequestWorker *worker, Attachment summary);
    void attachmentUpdateSignalFull(HttpRequestWorker *worker, Attachment summary);

    Q_DECL_DEPRECATED_X("Use attachmentCreateSignalError() instead")
    void attachmentCreateSignalE(Attachment summary, QNetworkReply::NetworkError error_type, QString error_str);
    void attachmentCreateSignalError(Attachment summary, QNetworkReply::NetworkError error_type, const QString &error_str);
    Q_DECL_DEPRECATED_X("Use attachmentDestroySignalError() instead")
    void attachmentDestroySignalE(QNetworkReply::NetworkError error_type, QString error_str);
    void attachmentDestroySignalError(QNetworkReply::NetworkError error_type, const QString &error_str);
    Q_DECL_DEPRECATED_X("Use attachmentListSignalError() instead")
    void attachmentListSignalE(PaginatedAttachmentList summary, QNetworkReply::NetworkError error_type, QString error_str);
    void attachmentListSignalError(PaginatedAttachmentList summary, QNetworkReply::NetworkError error_type, const QString &error_str);
    Q_DECL_DEPRECATED_X("Use attachmentMetadataPartialUpdateSignalError() instead")
    void attachmentMetadataPartialUpdateSignalE(QNetworkReply::NetworkError error_type, QString error_str);
    void attachmentMetadataPartialUpdateSignalError(QNetworkReply::NetworkError error_type, const QString &error_str);
    Q_DECL_DEPRECATED_X("Use attachmentMetadataRetrieveSignalError() instead")
    void attachmentMetadataRetrieveSignalE(QNetworkReply::NetworkError error_type, QString error_str);
    void attachmentMetadataRetrieveSignalError(QNetworkReply::NetworkError error_type, const QString &error_str);
    Q_DECL_DEPRECATED_X("Use attachmentMetadataUpdateSignalError() instead")
    void attachmentMetadataUpdateSignalE(QNetworkReply::NetworkError error_type, QString error_str);
    void attachmentMetadataUpdateSignalError(QNetworkReply::NetworkError error_type, const QString &error_str);
    Q_DECL_DEPRECATED_X("Use attachmentPartialUpdateSignalError() instead")
    void attachmentPartialUpdateSignalE(Attachment summary, QNetworkReply::NetworkError error_type, QString error_str);
    void attachmentPartialUpdateSignalError(Attachment summary, QNetworkReply::NetworkError error_type, const QString &error_str);
    Q_DECL_DEPRECATED_X("Use attachmentRetrieveSignalError() instead")
    void attachmentRetrieveSignalE(Attachment summary, QNetworkReply::NetworkError error_type, QString error_str);
    void attachmentRetrieveSignalError(Attachment summary, QNetworkReply::NetworkError error_type, const QString &error_str);
    Q_DECL_DEPRECATED_X("Use attachmentUpdateSignalError() instead")
    void attachmentUpdateSignalE(Attachment summary, QNetworkReply::NetworkError error_type, QString error_str);
    void attachmentUpdateSignalError(Attachment summary, QNetworkReply::NetworkError error_type, const QString &error_str);

    Q_DECL_DEPRECATED_X("Use attachmentCreateSignalErrorFull() instead")
    void attachmentCreateSignalEFull(HttpRequestWorker *worker, QNetworkReply::NetworkError error_type, QString error_str);
    void attachmentCreateSignalErrorFull(HttpRequestWorker *worker, QNetworkReply::NetworkError error_type, const QString &error_str);
    Q_DECL_DEPRECATED_X("Use attachmentDestroySignalErrorFull() instead")
    void attachmentDestroySignalEFull(HttpRequestWorker *worker, QNetworkReply::NetworkError error_type, QString error_str);
    void attachmentDestroySignalErrorFull(HttpRequestWorker *worker, QNetworkReply::NetworkError error_type, const QString &error_str);
    Q_DECL_DEPRECATED_X("Use attachmentListSignalErrorFull() instead")
    void attachmentListSignalEFull(HttpRequestWorker *worker, QNetworkReply::NetworkError error_type, QString error_str);
    void attachmentListSignalErrorFull(HttpRequestWorker *worker, QNetworkReply::NetworkError error_type, const QString &error_str);
    Q_DECL_DEPRECATED_X("Use attachmentMetadataPartialUpdateSignalErrorFull() instead")
    void attachmentMetadataPartialUpdateSignalEFull(HttpRequestWorker *worker, QNetworkReply::NetworkError error_type, QString error_str);
    void attachmentMetadataPartialUpdateSignalErrorFull(HttpRequestWorker *worker, QNetworkReply::NetworkError error_type, const QString &error_str);
    Q_DECL_DEPRECATED_X("Use attachmentMetadataRetrieveSignalErrorFull() instead")
    void attachmentMetadataRetrieveSignalEFull(HttpRequestWorker *worker, QNetworkReply::NetworkError error_type, QString error_str);
    void attachmentMetadataRetrieveSignalErrorFull(HttpRequestWorker *worker, QNetworkReply::NetworkError error_type, const QString &error_str);
    Q_DECL_DEPRECATED_X("Use attachmentMetadataUpdateSignalErrorFull() instead")
    void attachmentMetadataUpdateSignalEFull(HttpRequestWorker *worker, QNetworkReply::NetworkError error_type, QString error_str);
    void attachmentMetadataUpdateSignalErrorFull(HttpRequestWorker *worker, QNetworkReply::NetworkError error_type, const QString &error_str);
    Q_DECL_DEPRECATED_X("Use attachmentPartialUpdateSignalErrorFull() instead")
    void attachmentPartialUpdateSignalEFull(HttpRequestWorker *worker, QNetworkReply::NetworkError error_type, QString error_str);
    void attachmentPartialUpdateSignalErrorFull(HttpRequestWorker *worker, QNetworkReply::NetworkError error_type, const QString &error_str);
    Q_DECL_DEPRECATED_X("Use attachmentRetrieveSignalErrorFull() instead")
    void attachmentRetrieveSignalEFull(HttpRequestWorker *worker, QNetworkReply::NetworkError error_type, QString error_str);
    void attachmentRetrieveSignalErrorFull(HttpRequestWorker *worker, QNetworkReply::NetworkError error_type, const QString &error_str);
    Q_DECL_DEPRECATED_X("Use attachmentUpdateSignalErrorFull() instead")
    void attachmentUpdateSignalEFull(HttpRequestWorker *worker, QNetworkReply::NetworkError error_type, QString error_str);
    void attachmentUpdateSignalErrorFull(HttpRequestWorker *worker, QNetworkReply::NetworkError error_type, const QString &error_str);

    void abortRequestsSignal();
    void allPendingRequestsCompleted();

public Q_SLOTS:
    void tokenAvailable();
};

} // namespace InvenTree
#endif
